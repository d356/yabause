...
int GetNumberOfRequiredAccesses(int color_depth)
{
   if (color_depth == 0)//16
   {
      return 1;
   }
   else if (color_depth == 1)//256
   {
      return 2;
   }
   else if (color_depth == 2)//2048
   {
      return 4;
   }
   else if (color_depth == 3)//32768
   {
      return 4;
   }
   else if (color_depth == 4)//16 million
   {
      return 8;
   }
...
void CheckCyclePatterns()
{
   ...
   color_depth[0] = (Vdp2Regs->CHCTLA >> 4) & 7;
   color_depth[1] = (Vdp2Regs->CHCTLA >> 12) & 3;
